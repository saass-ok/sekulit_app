...
def render():
    ...
    if selected_id:
        ...
        if selected:
            # === Baris tombol close (di kanan atas) ===
            ...
            # === Kartu detail dengan padding lebih kecil ===
            st.markdown(f"""
            <div style="
                background:#ffffff;
                border-radius:16px;
                ...
                margin-bottom:12px;
            ">
                <h3 style="color:#4a3525; text-align:center; margin:0 0 12px 0;">{selected['name']}</h3>
            """, unsafe_allow_html=True)
            
            # Layout gambar (kiri) dan teks (kanan)
            col_img, col_text = st.columns([1, 2])
            with col_img:
                if os.path.exists(selected['image_url']):
                    st.image(selected['image_url'], use_container_width=True)
                else:
                    st.markdown(f"<div style='font-size:60px; text-align:center;'>{selected['icon']}</div>", unsafe_allow_html=True)
            
            with col_text:
                st.markdown(f"""
                <h4 style="color:#4a3525; margin:0 0 4px 0;">📋 Deskripsi</h4>
                <p style="color:#4a3525; opacity:0.9; margin:0 0 8px 0; font-size:13px;">{selected['description']}</p>
                <h4 style="color:#4a3525; margin:0 0 4px 0;">💊 Rekomendasi Obat</h4>
                <p style="color:#4a3525; opacity:0.9; margin:0 0 8px 0; font-size:13px;">{selected['obat']}</p>
                <h4 style="color:#4a3525; margin:0 0 4px 0;">🏥 Penanganan yang Tepat</h4>
                <p style="color:#4a3525; opacity:0.9; margin:0; font-size:13px;">{selected['penanganan']}</p>
                """, unsafe_allow_html=True)
    ...
    for idx, disease in enumerate(DISEASES):
        ...
        with col:
            ...
            if os.path.exists(disease['image_url']):
                ...
                st.markdown(f"<div style='font-size:60px; text-align:center;'>{disease['icon']}</div>", unsafe_allow_html=True)
            ...
            # Tombol Pilih
            if st.button("Pilih", key=f"btn_{disease['id']}", use_container_width=True, type="secondary"):
